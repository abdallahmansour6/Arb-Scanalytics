...
useful for understanding _why_ something used to be a certain way.
...
(batch) returning equivalent shapes is _not_ guaranteed. Probed
2026-05-02:
...
ms between _outgoing_ requests, regardless of how many tasks a caller
...
appear _twice_ in the tooltip. Drop `%{x}` from the template when
unified hover is on.
