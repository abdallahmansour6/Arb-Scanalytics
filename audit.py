...
NEXT_TS_TOKENS = ("nexttime", "next_time", "fundingtime", "funding_time",
                  "settletime", "settle_time", "settle_timestamp",
                  "fundinapply", "next_funding_apply", "fundingnextapply",
                  "nextupdate", "nextcollection", "nextsettle")
PREDICTED_TOKENS = ("nextfundingrate", "next_funding_rate", "expected",
                    "predicted", "predfunding", "estfunding", "estimated",
                    "indicative")
MARK_TOKENS = ("mark", "fairprice", "fair_price")
INDEX_TOKENS = ("index",)
OI_TOKENS = ("openinterest", "open_interest", "holding", "holdvol",
             "totalsize", "total_size", "oi")
VOL_TOKENS = ("turnover", "volume_24h_quote", "volume_24h_settle",
              "amount24", "value", "usdtvolume", "trade_turnover",
              "quote_volume")
...
async def audit_venue_symbol(canonical: str, sym: str) -> dict:
    async with open_client(canonical) as c:
        try:
            ...
            return {"venue": canonical, "symbol": sym,
        ...
        try:
            ...
            out["ticker_unified"] = {k: t.get(k) for k in
                ("last", "markPrice", "indexPrice", "bid", "ask",
        ...
        try:
            ...
            out["funding_unified"] = {k: f.get(k) for k in
                ("fundingRate", "nextFundingRate", "interval",
                 "fundingTimestamp", "nextFundingTimestamp", "markPrice",
                 "indexPrice")}
...
def print_findings(report: dict):
    ...
    print(f"  ticker.info keys ({len(report.get('ticker_info_keys', []))}): {report.get('ticker_info_keys', [])}")
    print(f"  funding.info keys ({len(report.get('funding_info_keys', []))}): {report.get('funding_info_keys', [])}")
